...
const clipSchema = z.object({ x: z.number(), y: z.number(), width: z.number(), height: z.number() });
...
function optClip(params: Params): z.infer<typeof clipSchema> | undefined {
  ...
  if (!parsed.success) throw new BrowseCommandError(`Expected { x, y, width, height } numbers for "clip".`);
  return parsed.data;
}
